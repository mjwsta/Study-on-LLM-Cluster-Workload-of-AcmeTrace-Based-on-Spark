...
spark = SparkSession.builder \
    .appName("KalosGPUUsageHistogram") \
...
# 表路径
table_paths = {
    "gpu_util_kalos": "/AcmeTrace/data/utilization/kalos/GPU_UTIL.csv",  # Kalos 集群的 GPU 利用率
}
...
# 加载 Kalos 集群 GPU 利用率数据
gpu_util_kalos_df = load_table(table_paths["gpu_util_kalos"])
...
# 获取 GPU 利用率的数值列
gpu_columns_kalos = get_numeric_columns(gpu_util_kalos_df)

# 计算 GPU 使用情况（每个节点的平均使用率）
gpu_avg_usage_kalos_df = gpu_util_kalos_df.select(*gpu_columns_kalos).agg(*[avg(col(c)).alias(c) for c in gpu_columns_kalos])

# 转换为 Pandas DataFrame 用于图表展示
gpu_avg_usage_kalos_pdf = gpu_avg_usage_kalos_df.toPandas()

# 获取所有节点的平均 GPU 使用率
usage_data = gpu_avg_usage_kalos_pdf.mean(axis=0)
...
# 保存图表为文件
output_image_path = "results/1.9.png"
