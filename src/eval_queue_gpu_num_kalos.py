...
# 表路径
table_paths = {
    "tasks_seren": "/AcmeTrace/data/job_trace/kalos_tasks.csv",  # Seren 集群任务
}
...
# 处理 GPU 使用个数区间和生成标签
gpu_bins = list(range(0, 1400, 100))  # GPU 使用个数的区间，例如 0-150, 150-300, ...
...
# 计算每个区间的平均排队时延
avg_queue_time_df = eval_tasks_seren_df.groupBy("GpuUseCountBin").avg("queue").orderBy("GpuUseCountBin")
...
ax.set_ylabel("eval avg queue time(s)", fontsize=12)
ax.set_xticklabels(gpu_bins, rotation=45, ha='right')

plt.tight_layout()

# 保存图像
output_path = "results/1.4.png"
